    ...
    <div class="well col-md-12 ">
        <form role="form" action="./donation" method="post" id="form">
                    ...
                    <select id="selectProvince" name="province" data-rel="chosen">
                        <%
                            while (iterator.hasNext()){
                                ...
                                if(value.equals(donation.getProvince())){
                                    donation.setProvince(key);
                                    out.print("<option value=\" "+ key + "\" selected>" + value + "</option>");
                                }
                                else{
            ...
            <c:forEach var="item" items="${cities}">
                <c:choose >
                    <c:when test="${ item.key eq donation.province}">
                        <div class="form-group  col-md-12 controls${item.key}">
                            <label class="control-label"  for="selectCity${item.key}"><h5>选择市（区）</h5></label>
                            <div>
                                <div class="controls"  >
                                    <select id="selectCity${item.key}" name="city"  data-rel="chosen" >
                                        <c:forEach var="i" items="${item.value}">
                                            <c:choose >
                                                <c:when test="${i.name eq donation.city}">
